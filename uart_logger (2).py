...
Thứ tự field gửi lên (khớp với đoạn code uart_send_int trong main loop):
  ...
  4. current_tick     (ms thô, dùng tính đạo hàm)
  5. prev_tick        (ms thô, dùng tính đạo hàm)
  6. v                (đạo hàm debug_v *100 -> chia 100, đơn vị %/s)
  7. turn_on_thres    (*100 -> chia 100)
  8. turn_off_thres   (*100 -> chia 100)
  9. peltier          (0/1, thô)
  10. cold_fan        (0/1 hoặc PWM%, thô)
  11. ntc_temp        (*100 -> chia 100)
  12. dew_point       (*100 -> chia 100, đã sửa lại theo firmware mới)
  13. cycle_extreme_hum (*100 -> chia 100, đỉnh/đáy filtered_hum của chu kỳ hiện tại — field mới, bước 2)
  14. overshoot_on_learned  (*100 -> chia 100, margin EMA học được cho chiều bật — field mới, bước 3)
  15. overshoot_off_learned (*100 -> chia 100, margin EMA học được cho chiều tắt — field mới, bước 3)
...
FIELDS = [
    "timestamp", "raw_hum", "filtered_hum", "current_tick", "prev_tick",
    "v", "turn_on_thres", "turn_off_thres", "peltier", "cold_fan",
...
SCALE_100 = {"raw_hum", "filtered_hum", "v", "turn_on_thres", "turn_off_thres",
             "ntc_temp", "dew_point", "cycle_extreme_hum",
             "overshoot_on_learned", "overshoot_off_learned"}
...
def main():
    ...
    fig, (ax_hum, ax_v, ax_state, ax_temp, ax_learn) = plt.subplots(
    ...
    def update(_frame):
        ...
        t = [(ts - buf["timestamp"][0]) / 1000.0 for ts in buf["timestamp"]]  # giây, mốc 0 = đầu window

        for ax in (ax_hum, ax_v, ax_state, ax_temp, ax_learn):
            ax.cla()
        ...
        ax_hum.legend(loc="upper right", fontsize=8, ncol=5)
        ax_hum.grid(alpha=0.3)

        # 2) Đạo hàm v
        ax_v.plot(t, buf["v"], color="tab:purple", lw=1.2)
        ...
        ax_temp.set_xlabel("t (s, cửa sổ gần nhất)")
